 ...
 */
class PurchaseSuccessController extends AbstractController implements PreCheckControllerInterface
{
    /**
    ...
    public function __invoke(string $gatewayName, string $token): Response
    {
        ...
        if ($payment->getState() === PaymentStateType::PENDING) {
            return new Response(
                $this->getTwig()->render('@DarvinPayment/payment/purchase_success.html.twig', [
                    'payment' => $payment,
                ])
            );
        }
        ...
        if ($response->isSuccessful()) {
            $this->getWorkflow()->apply($payment, Transitions::PURCHASE);
    ...
    public function preCheckPayment(GatewayInterface $gateway, Payment $payment): void
    {
        if (!$gateway->supportsCompletePurchase()) {
            $errorMessage = sprintf('Payment gateway %s doesn\'t support completePurchase method', $gateway->getName());
